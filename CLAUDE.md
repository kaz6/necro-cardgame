...
### 4.1 報告フォーマット — 完了項目と要判断項目はカードで出す

**完了した項目**と、**判定に迷った項目（人間の判断を仰ぐもの）**は、
散文で書かずに下記のカード形式で出力する。**正本である Notion へ
そのまま貼れることが目的**なので、Markdown のコードブロックに入れて出す。
...
- **対象**: （ファイル / ブランチ / コミット）
- **内容**: （何をしたか。1〜3行）
- **検証**: （どう確かめたか。未検証なら「未検証」と書く）
- **反映先**: （Notion のどのページに載るか）
```
...
- **選択肢**: A … ／ B …
- **推奨**: （推す案と理由。1〜2行）
- **決めないと止まるもの**: （この判断待ちで着手できない作業）
- **反映先**: （決定後 DECISION_LOG / REJECTED / SESSION_STATE のどれに書くか）
```

ルール:

- **勝手に決めた判断もカードで出す。** 指示のツリーから外れた追加、
  指示になかった項目の補完など、独断で動いた箇所は必ず要判断カードにして
  人間の承認を取れる形にする。黙って進めない。
- カード1枚 = 1項目。複数項目を1枚に詰めない。
- 未検証のものを「完了」と書かない。検証欄に「未検証」と明記する。
