...
### 4.2 反映先カード — 最後に必ず1枚出す

完了カード・要判断カードを出したら、**最後に「どこに書いたか」を1枚にまとめて出す。**
セッション中に何枚カードを出しても、**反映先カードは1枚だけ**。

これは 02_operations の運用規約が定める「共有欄」の本企画版。
カードが「裁定のための要約」なのに対し、こちらは**後から追うための手がかり**。
...
- **repo**: （ファイル / ブランチ / コミット）
- **サマリー**: （数行。何をしたか）
- **未反映・未対応**: （なければ「なし」と書く。空欄にしない）
```

ルール:

- **リンクは必ず実 URL で書く。** ページ名だけ書かない。コピペしてそのまま飛べる形にする。
- **実施日時は推測せず ****`TZ=Asia/Tokyo date`**** で取り直す。** チャット側は現在時刻を持たないため、
  報告の日時が時系列の基準になる。
- **Notion に書いていないものを「反映先」に書かない。** 未反映なら状態欄に「未反映」と明記する。
  規約上、Notion へ反映して初めて正本になる（反映前の `docs/` は未確定扱い）。
- **未反映・未対応の欄を空にしない。** なければ「なし」と書く。欄があれば自発的な申告に頼らず出てくる。
